...
class MemEncoder(nn.Module):
    ...
    def __init__(self, vocab, layers, embed_dim, ff_embed_dim, num_heads, dropout):
        ...
        self.src_pos_embed = SinusoidalPositionalEmbedding(embed_dim)
        self.embed_scale = math.sqrt(embed_dim)
        self.transformer = Transformer(layers, embed_dim, ff_embed_dim, num_heads, dropout, with_external=True)
        self.dropout = dropout

    def forward(self, src_input, mem_repr, mem_mask, mem_attn_bias):
        src_repr = self.embed_scale * self.src_embed(src_input) + self.src_pos_embed(src_input)
        src_repr = F.dropout(src_repr, p=self.dropout, training=self.training)
        src_mask = torch.eq(src_input, self.vocab.padding_idx)
        src_repr = self.transformer(src_repr, self_padding_mask=src_mask,
                                    external_memories=mem_repr, external_padding_mask=mem_mask,
                                    attn_bias=mem_attn_bias)
        return src_repr, src_mask
