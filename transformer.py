...
class Transformer(nn.Module):
    ...
    def __init__(self, layers, embed_dim, ff_embed_dim, num_heads, dropout, with_external=False, weights_dropout=True):
        ...
        for _ in range(layers):
            self.layers.append(TransformerLayer(embed_dim, ff_embed_dim, num_heads,
                                                dropout, with_external, weights_dropout))
    ...
    def forward(self, x, kv=None,
        ...
        for idx, layer in enumerate(self.layers):
            x, _, _ = layer(x, kv,
                            self_padding_mask, self_attn_mask,
                            external_memories, external_padding_mask, attn_bias)
        return x
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, kv=None,
                ...
                attn_bias=None, need_weights=None):
        ...
        if self.with_external:
            ...
            x, external_attn = self.external_attn(query=x, key=external_memories, value=external_memories,
                                                  ...
                                                  need_weights=need_weights, attn_bias=attn_bias)
            x = F.dropout(x, p=self.dropout, training=self.training)
